<?php
/**
 * @package SimplePlugin\Admin
 */

namespace SimplePlugin\Admin;

use SimplePlugin\Loader;
use SimplePlugin\Admin\Settings\General;
use SimplePlugin\Admin\Settings\About;
...
class Admin
{
    ...
    public function menu()
    {
        ...
        $general = add_menu_page(
            esc_html__( 'General Settings', 'simple-plugin' ),
            esc_html__( 'Plugin Name', 'simple-plugin' ),
            'manage_options',
            $general_slug,
            [ $general_page, 'render' ],
            'dashicons-smiley',
        ...
        $about = add_submenu_page(
            $general_slug,
            esc_html__( 'About Plugin Name', 'simple-plugin' ),
            esc_html__( 'About', 'simple-plugin' ),
            'manage_options',
            $about_slug,
            [ $about_page, 'render' ]
        );
        ...
        $submenu[ $general_slug ][0][0] = esc_html_x( 'General', 'settings screen', 'simple-plugin' );
    }
    ...
    public function plugins()
    {
        add_filter( 'plugin_action_links', function( $actions, $plugin_file, $plugin_data, $context ) {
            ...
            $new_actions['settings'] = sprintf( '<a href="%s">%s</a>', esc_url( $settings ), esc_html__( 'Settings', 'simple-plugin' ) );

            $about = add_query_arg( [
                'page' => sanitize_key( sprintf( '%s-about', $this->plugin_slug ) )
            ], network_admin_url( 'admin.php' ) );

            $new_actions['about'] = sprintf( '<a href="%s">%s</a>', esc_url( $about ), esc_html__( 'About', 'simple-plugin' ) );
        ...
        add_filter( 'plugin_row_meta', function( $plugin_meta, $plugin_file, $plugin_data, $status ) {
            ...
            $new_meta = [
                sprintf( '<a href="%s">%s</a>', esc_url( 'https://example.com/' ), esc_html__( 'Documentation', 'simple-plugin' ) ),
                sprintf( '<a href="%s">%s</a>', esc_url( 'https://example.com/wiki/' ), esc_html__( 'Wiki', 'simple-plugin' ) )
            ];
