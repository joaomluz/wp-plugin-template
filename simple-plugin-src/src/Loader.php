...
class Loader
{
    ...
    {
        if ( WP_DEBUG ) {
            trigger_error( __( 'Cloning is forbidden.', 'simple-plugin' ), E_USER_ERROR );
        }
    }
    ...
    {
        if ( WP_DEBUG ) {
            trigger_error( __( 'Unserializing instances of this class is forbidden.', 'simple-plugin' ), E_USER_ERROR );
        }
    }
    ...
    {
        load_plugin_textdomain( 'simple-plugin', false, dirname( $this->plugin_file ) . '/languages' );
    }
    ...
    public function admin()
    {
        ...
        if ( is_admin() ) {
            \SimplePlugin\Admin\Admin::newInstance( $this );
        }
    }
